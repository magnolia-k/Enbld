package Blender::Definition::Mysql;
...
sub set_versionlist {
    ...
    my $latest_html = Blender::HTTP->new( $indexsite_latest )->get_html;
...
sub archived_versions {
    ...
    my $major_html = Blender::HTTP->new( $index_site )->get_html;
    ...
    for my $major ( @{ $major_list } ) {
        my $revision_html = Blender::HTTP->new( $revision_site . $major )->get_html;
