...
=head2 perlのスクリプトとして実行
  ...
  -> 設定ファイルに従ってソフトウェアをインストールします。

=head1 説明

B<Enbldは、開発環境構築のための、もう一つのパッケージマネージャーです。>

ソフトウェアインストールの条件(バージョン - 最新か指定のバージョン、テストは実行するか 等)を設定ファイルに記述し、perlのスクリプトとして実行します。

するとEnbldは設定ファイルに従って、ソフトウェアをインストールします。
...
その後、より新しいバージョンがリリースされたら、もう一度実行して下さい。
...
バージョンの指定は、設定ファイルの中で行います。 -> version '5.18.1';
...
設定ファイルに指定するソフトウェアの名称は、この一覧に表示される名称を使います。

=head3 インストール済みのソフトウェアの表示

サブコマンド'list'は、インストール済みのソフトウェアと、バージョンを表示します。

 $ enblder list

=head3 インストール済みの設定の表示

サブコマンド'freeze'は、インストール済みのソフトウェアの設定を表示します。

 $ enblder freeze

テキストファイルにリダイレクトすれば、そのまま実行可能なperlのスクリプトになります。

 $ enblder freeze > configuration.pl
 $ chmod +x configuration.pl
 $ ./configuration.pl
