...
subtest 'getter method' => sub {
    ...
    subtest 'set make_test' => sub {
        my $condition = Blender::Condition->new( make_test => 1 );
...
subtest 'serialize' => sub {
    ...
    subtest 'modules serialize' => sub {
        my $condition = Blender::Condition->new( modules => {
                module_a    => 0,
                module_b    => 0,
                });
...
subtest 'is_equal_to method' => sub {
    ...
    subtest 'condition check' => sub {
        my $condition = Blender::Condition->new( version => 'latest' );
        my $param     = Blender::Condition->new( version => 'latest' );

        is( $condition->is_equal_to( $param ), $condition, 'same version' );

        my $param_diff = Blender::Condition->new( version => '1.0' );
    ...
    subtest 'make_test check' => sub {
        ...
        my $condition = Blender::Condition->new( make_test => 1 );
        my $param     = Blender::Condition->new( make_test => 1 );

        is( $condition->is_equal_to( $param ), $condition, 'same make_test' );

        my $param_diff = Blender::Condition->new( make_test => undef );

        is( $condition->is_equal_to( $param_diff ), undef,
                'different make_test' );

        my $condition_diff = Blender::Condition->new( make_test => undef );
    ...
    subtest 'modules check' => sub {
        ...
        my $condition = Blender::Condition->new( modules => { module => 0 } );
        my $param     = Blender::Condition->new( modules => { module => 0 } );

        is( $condition->is_equal_to( $param ), $condition, 'same modules' );

        my $param_diff = Blender::Condition->new( modules => undef );

        is( $condition->is_equal_to( $param_diff ), undef,
                'different modules pattern1' );

        my $condition_diff = Blender::Condition->new( modules => undef );
